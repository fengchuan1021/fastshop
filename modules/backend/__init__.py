...
async def checkpermission(db: AsyncSession,request: Request,token: settings.UserTokenData):
    ...
    if flag:
        ...
        while roleid:
            ...
            if roleid & i :
                ...
                if haspermission:
                    await cache.hset(f"xtadmin:rolepermission{token.userrole}",api_name,1)
                    await cache.setTtl(f"xtadmin:rolepermission{token.userrole}",3600*24)
                    return
                roleid=roleid -i

        await cache.hset(f"xtadmin:rolepermission{token.userrole}",api_name,0)
        await cache.setTtl(f"xtadmin:rolepermission{token.userrole}", 3600 * 24)
        raise PermissionException(msg="you dont have permission access this api")
