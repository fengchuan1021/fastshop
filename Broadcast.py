...
async def fireAfterUpdated(updatedModels:Iterable[Model],db: AsyncSession,token:settings.UserTokenData,background:bool=False)->None:
...
def BeforeModelCreated(listenModel : Type[Model])->Callable[[F], F]:
    ...
    def decorator(func:F)->F:
        ...
        return cast(F, wrapper)
    return decorator
async def fireBeforeCreated(newModels:Iterable[Model],db: AsyncSession,token:settings.UserTokenData)->None:
...
def AfterModelDeleted(listenModel:Type[Model],background:bool=False)->Callable[[F], F]:
    ...
    def decorator(func:F)->F:
        ...
        return cast(F, wrapper)
    return decorator
async def fireAfterDeleted(deletedModels:Iterable[Model],db:AsyncSession,token:settings.UserTokenData,background:bool=False)->None:
...
async def fireAfterCreated(newModels:Iterable[Model],db: AsyncSession,token:settings.UserTokenData,background:bool=False)->None:
    for model in newModels:
