...
class PurchaseReceipt(Base):
    ...
    purchase_receipt_id = Column(
        BIGINT, primary_key=True, default=snowFlack.getId, comment="primary key"
    )
    ...
    totalmoney = Column(
        ...
        default=0,
        server_default="0",
        comment="shipfee+otherfee+productmoney",
    )
    arrivein = Column(
        ENUM("1day", "3day", "5day", "7day", "9day", "15day"),
        comment="arrive in n days",
    )
    ...
    Supplier: "Supplier" = relationship(
        ...
        back_populates="PurchaseReceipt",
        cascade="",
    )
    ...
    PurchaseReceiptItems: List["PurchaseReceiptItems"] = relationship(
        "PurchaseReceiptItems",
        primaryjoin="foreign(PurchaseReceiptItems.purchase_receipt_id)==PurchaseReceipt.purchase_receipt_id",
        uselist=True,
        back_populates="PurchaseReceipt",
        cascade="",
    )
...
class PurchaseReceiptItems(Base):
    ...
    purchase_receipt_items_id = Column(
        BIGINT, primary_key=True, default=snowFlack.getId, comment="primary key"
    )
    merchant_id = Column(INTEGER, index=True)
    ...
    totalmoney = Column(
        DECIMAL(10, 2), default="0", server_default="0", comment="number*price"
    )

    PurchaseReceipt: "PurchaseReceipt" = relationship(
        "PurchaseReceipt",
        primaryjoin="foreign(PurchaseReceiptItems.purchase_receipt_id)==PurchaseReceipt.purchase_receipt_id",
        uselist=False,
        back_populates="PurchaseReceiptItems",
        cascade="",
    )
